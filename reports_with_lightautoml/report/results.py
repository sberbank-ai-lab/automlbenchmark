...
def load_results(files):
    return pd.concat([pd.read_csv(file, engine='python') for file in files], ignore_index=True)
...
def prepare_results(results_files,
    ...
    results = load_results(results_files).set_index(['task', 'fold', 'framework'])
    results = results[~results.index.duplicated(keep='last')].reset_index()
    ...
    done = done[~done.index.duplicated(keep='last')]
